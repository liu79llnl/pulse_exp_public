...
def prepare_facebank(conf, model, mtcnn, tta = True):
    ...
    for path in conf.facebank_path.iterdir():
            ...
            for file in path.iterdir():
                if not file.is_file():
                    ...
                    if img.size != (112, 112):
                        try:
                            img = mtcnn.align(img)
                            img.save(conf.facebank_path/f"{path.name}.png")
                        except:
                            print('excepting')
                            continue
                        if first_img is None:
                            first_img = np.array(img.getdata())
                            #print(first_img)
                            first_img = first_img.astype(np.float32)
                            #print(first_img)
                            first_img = first_img.reshape(img.size[0], img.size[1], 3)
                            #print(first_img)
                            first_img = cv2.resize(first_img, dsize=(128, 128), interpolation=cv2.INTER_CUBIC)
                    with torch.no_grad():
                        if tta:
        ...
        print(path.name)
        np.save(conf.facebank_path/path.name, first_img)
